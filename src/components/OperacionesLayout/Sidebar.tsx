...
export default function OperacionesSidebar() {
  ...
  return (
    <aside className={`sidebar ${isCollapsed ? 'collapsed' : ''}`}>
      ...
      <nav className="sidebar-nav" style={{ overflowY: 'auto' }}>
        {isCollaborator ? (
          <>
            <NavLink to="/operaciones/plan-trabajo" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} title="Plan de trabajo">
          ...
          <>
            <NavLink to="/operaciones/flujos" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} title="Flujos">
              <Workflow size={18} />
              <span>Flujos</span>
            </NavLink>
            <NavLink to="/operaciones/collaborators" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} title="Colaboradores">
              <UserCheck size={18} />
            ...
            <NavLink to="/operaciones/tags" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} title="Etiquetas">
              <Hash size={18} />
              <span>Etiquetas</span>
            </NavLink>

            <div className="nav-section">ADMINISTRACIÓN</div>
            <NavLink to="/operaciones/clients" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} title="Clientes">
              <Building2 size={18} />
              <span>Clientes</span>
            </NavLink>
            <NavLink to="/operaciones/admin" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} title="Usuarios">
              <UserPlus size={18} />
              <span>Usuarios</span>
            </NavLink>
          </>
